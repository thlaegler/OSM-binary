...
public class FileBlock extends FileBlockBase {
  ...
  public static FileBlock newInstance(String type, ByteString blob, ByteString indexdata) {
    if (blob != null && blob.size() > MAX_BODY_SIZE / 2) {
      System.err
          .println("Warning: Fileblock has body size too large and may be considered corrupt");
    ...
    if (indexdata != null && indexdata.size() > MAX_HEADER_SIZE / 2) {
      System.err
          .println("Warning: Fileblock has indexdata too large and may be considered corrupt");
  ...
  public FileBlockPosition writeTo(OutputStream outwrite, CompressFlags flags) throws IOException {
    BlobHeader.Builder builder = Fileformat.BlobHeader.newBuilder();
    if (indexdata != null)
    ...
    // System.out.format("Outputed header size %d bytes, header of %d bytes, and blob of %d
    // bytes\n",
