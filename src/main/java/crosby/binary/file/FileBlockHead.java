...
/**
 * Intermediate representation of the header of a fileblock when a set of fileblocks is read as in a
 * stream. The data in the fileblock must be either skipped (where the returned value is a reference
 * to the fileblock) or parsed.
 * 
 * @author crosby
 * 
 */
public class FileBlockHead extends FileBlockReference {
  ...
  static FileBlockHead readHead(InputStream input) throws IOException {
    ...
    if (header.getDatasize() > MAX_BODY_SIZE) {
      throw new FileFormatException(
          "Unexpectedly long body " + MAX_BODY_SIZE + " bytes. Possibly corrupt file.");
    }
  ...
  /**
   * Assumes the stream is positioned over at the start of the data, read it and return the complete
   * FileBlock
   * 
   * @throws IOException
   */
